(function () {
  ...
  const UI_NOISE = ["glissez", "drag", "drop", "fichiers", "here", "ici", "télécharger", "upload", "ajouter une", "add a"];

  function isUiPlaceholder(text) {
    const t = text.toLowerCase();
    return UI_NOISE.some(n => t.includes(n));
  }
    ...
    if (categories.length === 0) {
      ...
      for (const btn of deleteButtons) {
        ...
        if (container) {
          ...
          if (isUiPlaceholder(name)) continue;
          const countText = container.textContent.match(/(\d+)\s*image/);
          categories.push({ name, imageCount: countText ? parseInt(countText[1], 10) : 0 });
        }
      }
    }
  ...
  function extractTrainingState() {
    ...
    const isVerifyActive = verifyButton
      ...
      : false;
    return {
      trainButtonVisible: !!findElementByText("Entraîner le modèle"),
  ...
  function sendData(data) {
    ...
    lastDataHash = hash;
    try {
      chrome.runtime.sendMessage({ type: "LEARN_AI_DATA", data }).catch(() => {});
    } catch (e) {}
  }
  ...
  setInterval(() => {
    try {
      ...
      if (data.isDecliTechAI || data.exerciseInfo?.title) sendData(data);
    } catch (e) {}
  }, EXTRACT_INTERVAL);
  ...
  chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
    if (message.type === "REQUEST_LEARN_AI_DATA") {
      try {
