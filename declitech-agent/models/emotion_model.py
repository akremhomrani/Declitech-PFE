...
from utils import preprocess_fer, softmax
...
class EmotionModel:
    ...
    def predict(self, face_image: np.ndarray) -> Dict[str, float]:
        ...
        if len(probs) != len(settings.EMOTION_CLASSES):
            raise RuntimeError(
                f"Output size {len(probs)} != {len(settings.EMOTION_CLASSES)}"
            )
        
        return {
            settings.EMOTION_CLASSES[i]: float(probs[i])
            for i in range(len(settings.EMOTION_CLASSES))
        }
