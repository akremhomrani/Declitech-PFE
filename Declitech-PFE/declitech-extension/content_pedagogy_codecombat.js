...
function detectCodeCombat() {
    try {
        ...
        const codeEditorEl = document.querySelector(
            '.ace_content, .CodeMirror-code, #code-area, .code-editor .lines-content'
        );
        const studentCode = codeEditorEl?.innerText?.trim()?.slice(0, 500) || '';
