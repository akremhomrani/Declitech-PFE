...
export class EmotionService {
  ...
  /**
   * Single batch fetch — backend returns all alerts for the session, frontend aggregates
   * per identity. Avoids N+1 (one HTTP per student) on the session-details page.
   */
  getAllSessionAlerts(sessionId: string): Observable<SessionAlert[]> {
    return this.http.get<SessionAlert[]>(ApiPaths.alerts.session(sessionId), HTTP_WITH_CREDENTIALS);
  }
}
