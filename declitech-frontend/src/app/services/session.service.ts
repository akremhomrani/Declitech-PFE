...
export class SessionService {
  ...
  readonly sessionError$ = this.sessionErrorSubject.asObservable();
    ...
    this.http.post<CreateSessionResponse>(ApiPaths.sessions.root, request, HTTP_WITH_CREDENTIALS).subscribe({
      ...
      error: (err) => {
        ...
        const message = err?.status === 0
          ? 'SESSION_CREATION.ERROR_OFFLINE'
          : err?.status === 403
            ? 'SESSION_CREATION.ERROR_FORBIDDEN'
            : 'SESSION_CREATION.ERROR_GENERIC';
        this.sessionErrorSubject.next(message);
      }
    });
  }
